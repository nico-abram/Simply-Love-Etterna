	...
	if not (FailString and FailString == "FailOff") then
		...
		local danger = Def.Quad{
			...
			DangerCommand=function(self)
				self:linear(0.3):diffusealpha(0.7): diffuseshift():effectcolor1(color("1,0,0.24,0.3")):effectcolor2(color("1,0,0,0.8"))
			end,
			DeadCommand=function(self)
				self:diffusealpha(0): stopeffect(): stoptweening():diffuse(color("1,0,0")):linear(0.3):diffusealpha(0.8):linear(0.3):diffusealpha(0)
			end,
			OutOfDangerCommand=function(self)
				self:diffusealpha(0): stopeffect(): stoptweening():diffuse(color("0,1,0")):linear(0.3):diffusealpha(0.8):linear(0.3):diffusealpha(0)
			end,
			HideCommand=function(self)
				self:stopeffect(): stoptweening():linear(0.3):diffusealpha(0)
			end
		}
