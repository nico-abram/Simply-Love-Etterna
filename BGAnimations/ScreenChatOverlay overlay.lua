...
local width, height = SCREEN_WIDTH, SCREEN_HEIGHT * 0.05
local maxlines = 5
local lineNumber = 5
...
for i = 0, maxTabs - 1 do
    chatWindow[#chatWindow + 1] =
        ...
        Def.Quad {
            ...
            UpdateChatOverlayMessageCommand = function(self)
                self:diffuse(
                    (tabs[i + 1] and currentTabName == tabs[i + 1][2] and currentTabType == tabs[i + 1][1]) and
                        Colors.activeTab or
                        Colors.tab
                )
                    ...
                    if
                        tabs[i + 1] and
                            ((tabs[i + 1][1] == 0 and tabs[i + 1][2] == "") or
                                (tabs[i + 1][1] == 1 and tabs[i + 1][2] ~= nil and
                                    tabs[i + 1][2] == NSMAN:GetCurrentRoomName()))
                     then
                        self:settext("")
                    else
                        self:settext("X")
            ...
            if not tabButton then
                ...
                if event.type == "InputEventType_FirstPress" then -- only change tabs on press (to stop repeatedly closing tabs or changing to a tab we close) -poco
                    if not closeTab then
                        ...
                        if
                            (tabT == 0 and tabN == "") or
                                (tabT == 1 and tabN ~= nil and tabN == NSMAN:GetCurrentRoomName())
                         then
                            return false
                        end
                        tabs[tabButton] = nil
