	...
	if EarnedMachineRecord or EarnedPersonalRecord then
		...
		if EarnedMachineRecord then
			t[#t+1] = LoadFont("_wendy small")..{
				...
				InitCommand=function(self)
					self:halign(0):xy(-110,-18):diffuse(PlayerColor(player)): glowshift():effectcolor1(color("1,1,1,0")):effectcolor2(color("1,1,1,0.25"))
				end,
			}
		end
		...
		if EarnedPersonalRecord then
			t[#t+1] = LoadFont("_wendy small")..{
				...
				InitCommand=function(self)
					self:halign(0):xy(-110,24):diffuse(PlayerColor(player)): glowshift():effectcolor1(color("1,1,1,0")):effectcolor2(color("1,1,1,0.25"))
				end,
			}
		end
