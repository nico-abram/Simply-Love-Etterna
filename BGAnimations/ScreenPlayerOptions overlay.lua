	...
	if mods.SpeedModType == "x" then
		...
		if bpm[1] == bpm[2] then
			...
			display =
				...
				"x (" .. round(speed * bpm[1] * musicrate) .. " - " .. round(speed * bpm[2] * musicrate) .. ")"
		end
	...
	CaptureCommand = function(self)
		...
		for player in ivalues(GAMESTATE:GetHumanPlayers()) do
			...
			if SpeedModRowIndex then
				...
				local item = ScreenOptions:GetOptionRow(SpeedModRowIndex):GetChild(""):GetChild("Item")
				SpeedModItems[pn] = SL.IsEtterna and item or item[PlayerNumber:Reverse()[player] + 1]
...
local GetNoteSkinActor = function(ns)
	local status, noteskin_actor =
		pcall(NOTESKIN.LoadActorForNoteSkin, NOTESKIN, column[game_name] or "Up", "Tap Note", ns)
