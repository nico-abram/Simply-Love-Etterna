...
for i = 1, NumDifficulties do
	DifficultiesToShow[#DifficultiesToShow + 1] =
		ToEnumShortString(THEME:GetMetric("ScreenRankingSingle", "ColumnDifficulty" .. i))
end
		...
		for index, steps in pairs(params.Entries) do
			if MachineProfile and steps then
				...
				if top_score then
					...
					self:GetChild("HighScore_" .. difficulty) --[[:hibernate(100)]]:y(math.huge):visible(false)
				end
			end
		end
	end
